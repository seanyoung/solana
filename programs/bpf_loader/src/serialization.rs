...
impl Serializer {
    ...
    fn write_account(
        ...
        let vm_data_addr = if self.copy_account_data {
            ...
            if !account.serialize_this_one(program_id, feature_set) {
                self.write_all(&[]);
            } else {
        ...
        if self.aligned {
            let data_len = if !account.serialize_this_one(program_id, feature_set) {
                0
            } else {
    ...
    for account in &accounts {
        ...
        match account {
            ...
            SerializeAccount::Account(_, account) => {
                ...
                if copy_account_data && account.serialize_this_one(program_id, feature_set) {
...
pub fn deserialize_parameters_unaligned<I: IntoIterator<Item = usize>>(
        ...
        .zip(account_lengths.into_iter())
    {
        ...
        if duplicate.is_none() {
            ...
            if copy_account_data && borrowed_account.serialize_this_one(program_id, feature_set) {
                let data = buffer
    ...
    for account in accounts {
        match account {
            SerializeAccount::Account(_, mut borrowed_account) => {
                ...
                if !borrowed_account.serialize_this_one(program_id, feature_set) {
                    s.write::<u64>(0u64.to_le());
                } else {
                    s.write::<u64>((borrowed_account.get_data().len() as u64).to_le());
                }
                let vm_data_addr = s.write_account(program_id, &mut borrowed_account, feature_set)?;
...
pub fn deserialize_parameters_aligned<I: IntoIterator<Item = usize>>(
        ...
        .zip(account_lengths.into_iter())
    {
        ...
        if duplicate.is_some() {
            ...
            // The redundant check helps to avoid the expensive data comparison if we can
            let alignment_offset = (if !borrowed_account.serialize_this_one(program_id, feature_set) {
                0
            } else {
                pre_len
            } as *const u8)
            ...
            } else if borrowed_account.serialize_this_one(program_id, feature_set) {
