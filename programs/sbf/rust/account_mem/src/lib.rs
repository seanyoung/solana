...
pub fn process_instruction(
    ...
    match instruction_data[0] {
        0 => {
            // memcmp overlaps end
            sol_memcmp(&buf, &data[data_len - 8..], 16);
        }
        1 => {
            // memcmp overlaps end
            sol_memcmp(&data[data_len - 7..], &buf, 15);
        }
        2 => {
        ...
        4 => {
            ...
            sol_memset(&mut data[data_len - 2..], 0, 3);
        }
        5 => {
        ...
        6 => {
            ...
            sol_memcpy(&mut buf, &data[data_len - 3..], 10);
        }
        7 => {
            // memmov src overlaps end of account
            unsafe { sol_memmove(buf.as_mut_ptr(), data[data_len - 3..].as_ptr(), 10) };
        }
        8 => {
        ...
        10 => {
            ...
            sol_memcpy(&mut data[data_len - 3..], &buf, 10);
        }
        11 => {
            // memmov dst overlaps end of account
            unsafe { sol_memmove(data[data_len - 3..].as_mut_ptr(), buf.as_ptr(), 10) };
        }
        12 => {
